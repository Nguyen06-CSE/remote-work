...
public sealed class ActivityCollector : IActivityCollector
{
    ...
    public ActivityCollector(
        ...
        IdleActivityCollector idleCollector,
        KeyboardActivityCollector keyboardCollector,
        MouseActivityCollector mouseCollector,
    ...
    public IReadOnlyList<ActivityEvent> Collect()
    {
        ...
        // Xử lý thời điểm bắt đầu cho việc tính duration (tránh tính thời gian trước khi collector chạy)
    ...
    public ActivityBatch FlushBatch()
    {
        ...
        // Reset bộ tích lũy và mốc thời gian cho batch tiếp theo
        _accumulator.Reset();
        _batchStartedAt = now;
        _lastTimestamp = now;
