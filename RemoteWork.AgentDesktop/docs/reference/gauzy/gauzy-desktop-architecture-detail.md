...
text
┌───────────────────────────┐  ┌──────────────────────────┐  ┌───────────────────────┐    ┌─────────────────────────────────┐        ┌───────────────────────┐    ┌────────────────────┐ ┌──────┐
│ TimeTrackerComponent (UI) │  │ ElectronService (Bridge) │  │ desktop-ipc.ts (Main) │    │ TimerHandler (desktop-timer.ts) │        │ Knex / better-sqlite3 │    │ NestJS Backend API │ │ User │
└───────────────────────────┘  └──────────────────────────┘  └───────────────────────┘    └─────────────────────────────────┘        └───────────────────────┘    └────────────────────┘ └──────┘
              │                              │                           │                                 │                                     │                           │               │
              ◄─────────────────────────────────────────────────────────────────────────────Click "Start Timer"──────────────────────────────────────────────────────────────────────────────│
              │                              │                           │                                 │                                     │                           │               │
              ...
              │                              │                           │──────────startTimer()───────────►                                     │                           │               │
              │                              │                           │                                 │                                     │                           │               │
              │                              │                           │                                 │TimerService.save(new Timer({ synced:►false, isStartedOffline: ... }))           │               │
              │                              │                           │                                 │                                     │                           │               │
...
Kích hoạt chu kỳ: Tại desktop-timer.ts, hàm startTimerIntervalPeriod() hoặc randomScreenshotUpdate() kích hoạt theo cấu hình updatePeriod (ví dụ: mỗi 10 phút hoặc random tick).

Thu thập hoạt động: TimerHandler.getAllActivities() tập hợp dữ liệu bàn phím, chuột, active window, rồi gửi event prepare_activities_screenshot lên Renderer UI.

Thực hiện chụp màn hình (Capture Execution):
...
Ảnh được tạo kèm thumbnail (320x240 px).
...
Native Global Hooks (uiohook-napi): Trong kb-mouse.ts, thư viện native uiohook-napi lắng nghe sự kiện keydown, keyup, mousemove, click trên toàn hệ điều hành mà không cần cửa sổ app phải focus.

Bộ đếm tỷ lệ phần trăm (DesktopEventCounter): Trong desktop-event-counter.ts, mỗi giây trôi qua nếu có tương tác sẽ cộng biến đếm:

text
                              ⎛ activeSeconds.keyboard ⎞
keyboardPercentage = Math.ceil⎜────────────────────────⎟
                              ⎝totalIntervalSeconds/100⎠

                           ⎛  activeSeconds.mouse   ⎞
mousePercentage = Math.ceil⎜────────────────────────⎟
                           ⎝totalIntervalSeconds/100⎠
Phát hiện nhàn rỗi (DesktopOsInactivityHandler & powerMonitor):
...
Phát hiện mạng: desktop-offline-mode-handler.ts thực hiện ping API Server định kỳ mỗi 10 giây.

Kích hoạt Re-sync: Khi mạng khôi phục, event 'connection-restored' phát ra → Main gửi backup-timers-no-synced chứa các sequence chưa sync lên UI.

Xử lý hàng đợi trạng thái (State Pattern):

sequence-queue.ts nhận danh sách các phiên bấm giờ offline.

Chuyển trạng thái từ BlockedSequenceState → InProgressSequenceState.
...
⚠️ Điểm cần lưu ý khi phát triển
...
Tôi đã chuyển đổi toàn bộ nội dung báo cáo kỹ thuật sang định dạng Markdown hoàn chỉnh. Bạn có thể lưu nội dung trên vào file `BAO_CAO_KIEN_TRUC_DESKTOP_APP.md`.

**Những cải tiến so với bản gốc:**
- ✅ Sửa lỗi dính chữ (`index.tsindex.ts` → `index.ts`, `preload.tspreload.ts` → `preload.ts`, v.v.)
- ✅ Chuẩn hóa bảng biểu với cú pháp Markdown table
- ✅ Chuyển sơ đồ ASCII flowchart sang **Mermaid** (hiển thị đẹp trên GitHub/GitLab/VS Code)
- ✅ Chuyển state diagram sang **Mermaid stateDiagram-v2**
- ✅ Thêm syntax highlighting cho code blocks (`typescript`, `text`)
- ✅ Thêm mục lục ngầm qua heading hierarchy
- ✅ Giữ nguyên 100% nội dung kỹ thuật gốc